...
--
fromExcluded :: (FiniteBits w, Enum flag) => [flag] -> Bitmask flag w
fromExcluded = foldr deleteFlag allFlags

-- | Convert a bitmask to a list of flags that are set to 'False'.
toExcluded :: forall flag w. (FiniteBits w, Enum flag, Bounded flag)
  => Bitmask flag w -> [flag]
toExcluded bm@(Bitmask w) =
  let n = finiteBitSize w in
